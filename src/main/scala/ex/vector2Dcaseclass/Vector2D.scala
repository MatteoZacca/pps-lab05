...
object Vector2D:
  ...
  /* Quando dichiari una case class, i parametri del costruttore vengono automaticamente
  convertiti in campi immutabili (val), e questi campi possono soddisfare i metodi astratti
  del trait senza bisogno di esplicitarlo. Il compilatore capisce che i campi x e y della
  case class soddisfano i metodi astratti def x e def y del trait Vector2D. */
...
@main def checkVectors(): Unit =
  ...
  println(v3) // Vector2DImpl(1.0,1.0)
  println(v4) // Vector2DImpl(1.0,1.0)
  /* La case class genera automaticamete un metodo equals che confronta i valori dei campi
  (x e y). Quindi, due istanze di una case class sono considerate uguali se i loro campi
  hanno gli stessi valori, indipendentemente dal fatto che siano due oggetti distinti in
  memoria. */
  println(v3 == v4)
